...
async function startelaina() {
    ...
    // Group Update
    elaina.ev.on('groups.update', async pea => {
       //console.log(pea)
    // Get Profile Picture Group
       ...
       elaina.send5ButImg(pea[0].id, `「 Group Settings Change 」\n\nGroup telah ditutup oleh admin, Sekarang hanya admin yang dapat mengirim pesan !`, `Group Settings Change Message`, wm_fatih, [])
...
Tol = await getBuffer(`https://hardianto.xyz/api/goodbye3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/045471598515af3cbed67.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
    ...
    /** Send List Messaage
      ...
      *@param {*} quoted
      */
        elaina.sendListMsg = (jid, text = '', footer = '', title = '' , butText = '', sects = [], quoted) => {
        let sections = sects
        var listMes = {
        text: text,
        footer: footer,
        ...
        elaina.sendMessage(jid, listMes, { quoted: quoted })
        }
     ...
     * @returns 
     */
        elaina.send5ButMsg = (jid, text = '' , footer = '', but = []) =>{
        ...
        elaina.sendMessage(jid, templateMessage)
        }
